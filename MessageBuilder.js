...
class Button extends BaseBuilder {
    ...
    async build(jid, { ...options } = {}) {
        ...
        return generateWAMessageFromContent(
            jid,
            { ...this._extraPayload, interactiveMessage: { ...message, contextInfo: this._contextInfo } },
            { ...options }
        );
    }
...
class ButtonV2 extends BaseBuilder {
    ...
    setThumbnail(path) { this._image = path; return this; }
    setMedia(obj) { this._data = obj; return this; }
...
class AIRich extends BaseBuilder {
    ...
    addSubmessage(submessage) {
        ...
        for (const item of items) {
            if (typeof item !== "object" || item === null || Array.isArray(item)) throw new TypeError("Submessage must be a plain object or array");
    ...
    addSection(section) {
        ...
        for (const item of items) {
            if (typeof item !== "object" || item === null || Array.isArray(item)) throw new TypeError("Section must be a plain object or array");
    ...
    addText(text, { hyperlink = true, citation = true, latex = true } = {}) {
        ...
        const inline_entities = extractedIE.ie.map(({ type, ie }) => {
            ...
            return { key: ie.key, metadata: { latex_expression: ie.text, latex_image: { url: ie.url, width: 100, height: 100 }, font_height: 83.333333333333, padding: 15, __typename: "GenAILatexItem" } };
        });
    ...
    addImage(imageUrl) {
        ...
        const imageUrls = Array.isArray(imageUrl)
            ? imageUrl.map((url) => ({ imagePreviewUrl: url, imageHighResUrl: url, sourceUrl: "https://aljesat.bot/" }))
            : [{ imagePreviewUrl: imageUrl, imageHighResUrl: imageUrl, sourceUrl: "https://aljesat.bot/" }];

        this._submessages.push({
            messageType: 1,
            gridImageMetadata: { gridImageUrl: { imagePreviewUrl: Array.isArray(imageUrl) ? imageUrl[0] : imageUrl }, imageUrls },
        });
    ...
    addVideo(videoUrl) {
        ...
        const videoUrls = (Array.isArray(videoUrl) ? videoUrl : [videoUrl]).map((item) => {
            ...
            return { videoPreviewUrl: url, videoHighResUrl: url, duration: Number(duration) || 0, sourceUrl: "https://aljesat.bot/" };
        });
